...
---
```{r setup, include=FALSE, messageFALSE}
...
## Data
Ingold and Leifeld (2016) looked a four different policy networks for this analysis at two different time points in policy cycle stage. For the the purposes of our project we will look both decision-making policy networks and collaborative implementation networks in Switzerland. 
...
#### ERGM
```{r, message = FALSE}
climate1 <- ergm(climate1_reputation ~ edges + cyclicalties + transitiveties + mutual + nodeocov("odeg-rep") + edgecov(climate1_collab) + 
                   nodeicov("bc-perc") + nodeifactor("dm") + nodeicov("committees"), 
...
```{r, message = FALSE}
clim.lsm.1 <- latentnet::ergmm(climate1_reputation ~ euclidean(d=2, G = 2) + edges + nodeocov("odeg-rep") + 
...
### Network 1: Climate Policy Time 2 : 2002 - 2005 
...
fits.2 <-lapply(1:2, function(G){
    ergmm(climate2_reputation ~ euclidean(d=2, G=G) + edges + nodeocov("odeg-rep") + edgecov(climate2_collab) + nodeicov("bc-perc-2") + nodeifactor("dm") + nodeicov("committees-2"), seed = 12345)
  })
...
```{r, message = FALSE}
clim.lsm.2 <- latentnet::ergmm(climate2_reputation ~ euclidean(d=2, G=2) + edges + nodeocov("odeg-rep") + edgecov(climate2_collab) + 
                      nodeicov("bc-perc-2") + nodeifactor("dm") + nodeicov("committees-2"), seed = 12345)
summary(clim.lsm.2)
plot(clim.lsm.2)
```

The second wave of analysis on the climate change policy uses the same variables and yields similar results. In both our latent space and ergm have a significant edges coefficient. We also find similarly negative and positive coefficients on cyclical and transitive ties and again the coefficient on cyclical ties is significant while the coefficient on transitive ties is not. We find that the outdegree centrality of ego, presence of existing collaboration and betweenness centrality of the alter all have positive coefficients and are significant. Overall the results for the second wave of data mirror that of the first wave. This is unsurprising and tells us that the nature of the hierarchical and structural networks have not changed over time.  
...
tele.fit <-lapply(1:2, function(G){
    ergmm(tele_reputation ~ euclidean(d=2, G=G) + edges + nodeocov("odeg-rep") + edgecov(tele_collab) +
...
```{r, message = FALSE}
clim.lsm.2 <- ergmm(climate2_reputation ~ euclidean(d=2, G=2) + edges + nodeocov("odeg-rep") + edgecov(climate2_collab) + 
                      nodeicov("bc-perc-2") + nodeifactor("dm") + nodeicov("committees-2"), seed = 12345)
summary(clim.lsm.2)
plot(clim.lsm.2)
```
...
# SWISS CLIMATE CHANGE 1 
names.1 <- network::get.vertex.attribute(climate1_reputation, "vertex.names")
between_climate1_collab <- 100 * sna::betweenness(climate1_collab, rescale = TRUE)
committee.1 <- rowSums(com.1)
type.1 <- read.csv2("data/input/climate9500-type.csv", row.names = 1, header = TRUE)
appendix.1 <- data.frame(org = names.1, type3 = type.1[, 1], type5 = type.1[, 2], committees = committee.1, dm = dm.1$dm, betweenness = between_climate1_collab)

# SWISS CLIMATE CHANGE 2
names.2 <- network::get.vertex.attribute(climate2_reputation, "vertex.names")
between_climate2_collab <- 100 * sna::betweenness(climate2_collab, rescale = TRUE)
committee.2 <- rowSums(com.2)
type.2 <- read.csv2("data/input/climate0205-type.csv", row.names = 1, header = TRUE)
appendix.2 <- data.frame(org = names.2, type3 = type.2[, 1], 
    type5 = type.2[, 2], committees = committee.2, dm = dm.2$dm, 
    betweenness = between_climate2_collab)

# sTELECOMMUNICATIONS
names.3 <- network::get.vertex.attribute(tele_reputation, "vertex.names")
between_collab.3 <- 100 * sna::betweenness(tele_collab, rescale = TRUE)
type.3 <- read.csv2("input/telecom-type.csv", row.names = 1, header = TRUE)
